 ...
 *          names to avoid ABA hazards from pointer reuse.
...
typedef struct OradpiAsyncEntry {
    ...
    int            refcnt; /* reference count protected by gAsyncMutex */
    int            running;
    int            done;
    int            canceled;
    int            joined;
    int            orphaned; /* set by waiter on timeout; worker self-cleans */
    int            rc;
    int            errorCode;
    char          *errorMsg;
    /* persist dpiErrorInfo fields needed by orawaitasync so it
    ...
    /* Origin interp that enqueued this async operation.  Used by
     * CancelAndJoinAllForConn to scope cancellation to the tearing-down
     * interp instead of the entire shared connection, and to ensure
     * PendingsForget runs against the correct interp's pending map. */
    Tcl_Interp    *originIp;
...
typedef struct PoolWorkItem {
    char                *stmtKey; /* owned copy of registry key (not a raw pointer) */
...
static Tcl_HashTable        gAsyncByKey; /* string-keyed, not pointer-keyed */
...
/* =========================================================================
 * Async registry — uses stable string keys (handle names)
...
static OradpiAsyncEntry *AsyncLookup(const char *key) {
    ...
    /* increment refcount under gAsyncMutex so the entry cannot
...
static OradpiAsyncEntry *AsyncEnsure(const char *key, int *isNewOut) {
    ...
    if (isNew) {
        ...
        ae->refcnt = 2; /* one ref for the hash table, one for the caller */
        Tcl_SetHashValue(he, ae);
    } else {
        ae = (OradpiAsyncEntry *)Tcl_GetHashValue(he);
        ae->refcnt++; /* caller gets a ref */
    }
    Tcl_MutexUnlock(&gAsyncMutex);
    if (isNewOut)
        *isNewOut = isNew;
...
/* Free the entry's resources.  Only called when refcount reaches 0. */
...
/* Decrement refcount.  When it hits 0, free the entry.
...
/* Remove the entry from the hash table and release the table's
...
static void PoolThreadProc(void *cd) {
    ...
    for (;;) {
        ...
        /* work item carries a string key, not a raw pointer */
...
static void PoolEnsure(void) {
    ...
    /* only mark pool as initialized if at least one worker was
...
static void PoolEnqueue(const char *key) {
    /* enqueue a copy of the string key, not a raw pointer */
...
static void PoolExitHandler(void *unused) {
    ...
    /* Two-phase break — collect (shared, conn) pairs under
        ...
        for (e = Tcl_FirstHashEntry(&gAsyncByKey, &hs); e; e = Tcl_NextHashEntry(&hs)) {
            ...
            if (ae->running && !ae->done && ae->conn) {
                ...
                if (breakCount >= breakCap) {
                    ...
                    breakList  = (BreakItem *)Tcl_Realloc((char *)breakList, (unsigned)newCap * sizeof(BreakItem));
                    breakCap   = newCap;
                }
...
registry_cleanup:
    /* Clean up async registry entries at process exit.
 ...
 *
 * Contract: Worker threads MUST NOT call any Tcl_Interp-bound API.
...
static void AsyncWorkerBody(const char *key) {
    ...
    for (attempt = 0; attempt < totalTries; attempt++) {
        ...
        if (attempt + 1 < totalTries && errClasses) {
            ...
            /* Clamp to non-negative (NaN or negative backoffFact) */
            if (!(sleepMs >= 0.0))
                sleepMs = 0.0;
    ...
    /* persist recoverability so orawaitasync can fire failover callbacks */
    ...
    /* If the waiter timed out and set orphaned, the worker is
    ...
    /* release the reference obtained by AsyncLookup at entry */
    AsyncRelease(ae);
}
...
int Oradpi_Cmd_ExecAsync(void *cd, Tcl_Interp *ip, Tcl_Size objc, Tcl_Obj *const objv[]) {
    ...
    if (PoolThreadCount() == 0)
        return Oradpi_SetError(ip, (OradpiBase *)s, -1, "failed to create async thread pool (Tcl_CreateThread failed; check system thread limits)");

    /* use stable string key (handle name) instead of raw pointer */
    ...
    if (!isNew) {
        ...
        Tcl_MutexUnlock(&ae->lock);
        if (stillRunning) {
            AsyncRelease(ae); /* release ref from AsyncEnsure */
            return Oradpi_SetError(ip, (OradpiBase *)s, -1, "statement already executing asynchronously");
        }
        AsyncRemove(key);
        AsyncRelease(ae); /* release ref from first AsyncEnsure */
    ...
    /* Record the interp that owns this async operation so teardown
     * cancellation can be scoped to the correct interp. */
    ae->originIp        = ip;
    ...
    /* release caller's ref; the hash table still holds one,
...
int Oradpi_Cmd_WaitAsync(void *cd, Tcl_Interp *ip, Tcl_Size objc, Tcl_Obj *const objv[]) {
    ...
    if (objc == 4) {
        ...
        if (Tcl_GetWideIntFromObj(ip, objv[3], &wTimeout) != TCL_OK)
            return TCL_ERROR;
        /* reject negative values < -1 as invalid */
        if (wTimeout < -1) {
    ...
    /* use string key instead of raw pointer */
    const char       *key = Tcl_GetString(s->base.name);
    ...
    if (timeoutMs >= 0) {
        ...
        /* Normalizing loop handles usec values up to ~1999999
    ...
    if (ae->errorMsg) {
        /* pass NULL interp to avoid calling Tcl_SetObjResult
    ...
    if (rc != 0) {
        /* Reconstruct a minimal dpiErrorInfo on the interp thread
    ...
    /* return the actual Oracle/ODPI error code on failure, not the
...
int Oradpi_StmtWaitForAsync(OradpiStmt *s, int cancel, int timeoutMs) {
    ...
    if (timeoutMs >= 0) {
        ...
        deadline.sec += timeoutMs / 1000;
        deadline.usec += (timeoutMs % 1000) * 1000;
        /* Normalizing loop (same fix as WaitAsync) */
        while (deadline.usec >= 1000000) {
            deadline.sec++;
            deadline.usec -= 1000000;
        }
        ...
        if (!ae->done && ae->running) {
            /* mark as orphaned so the worker self-cleans.
...
void Oradpi_CancelAndJoinAllForConn(Tcl_Interp *ip, OradpiConn *co) {
    ...
    /* collect string keys instead of raw pointers.
     * also collect refcounted ae pointers so we don't need
    ...
    for (e = Tcl_FirstHashEntry(&gAsyncByKey, &hs); e; e = Tcl_NextHashEntry(&hs)) {
        ...
        /* acquire ae->lock while holding gAsyncMutex (respecting
         ...
         *
         * Also check ae->originIp — only cancel async entries that were
         * enqueued by the interp being torn down, so one wrapper's teardown
         * does not cancel async work started by a different interp/thread on the same physical
         * session.  Connection-wide break is reserved for final physical
         * connection teardown (refCount == 0 in SharedConnRelease). */
        Tcl_MutexLock(&ae->lock);
        if (ae->shared != co->shared || ae->originIp != ip) {
            Tcl_MutexUnlock(&ae->lock);
            continue;
        }
        ...
        if (count == cap) {
            ...
            if (cap > TCL_SIZE_MAX / 2 || Oradpi_CheckedAllocBytes(NULL, (newCap = cap * 2), sizeof(char *), &newKeysBytes, "async cancel key table") != TCL_OK ||
                Oradpi_CheckedAllocBytes(NULL, newCap, sizeof(char *), &newKeyNamesBytes, "async cancel statement-key table") != TCL_OK ||
                Oradpi_CheckedAllocBytes(NULL, newCap, sizeof(OradpiAsyncEntry *), &newEntriesBytes, "async cancel entry table") != TCL_OK) {
                Tcl_MutexUnlock(&ae->lock);
                continue;
            }
         ...
         * pass NULL interp — same deadlock avoidance. */
        if (ae->stmtKey) {
        ...
        /* hold a ref so the entry survives until we process it */
        ae->refcnt++;
        entries[count] = ae;
     ...
     * the orphan mechanism ensures the worker self-cleans. */
    for (Tcl_Size i = 0; i < count; i++) {
        if (keys[i]) {
            ...
            AsyncRelease(ae); /* release the ref from the scan loop */

            /* Only release bind-side pending refs when the worker
