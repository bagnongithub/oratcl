...
static int SnapshotDescribeCols(Tcl_Interp *ip, OradpiConn *co, dpiStmt *stmt, uint32_t ncols, OradpiDescCol *cols) {
    ...
    for (uint32_t i = 1; i <= ncols; i++) {
        ...
        if (Oradpi_CheckedAllocBytes(NULL, (Tcl_Size)qi.nameLength + 1, sizeof(char), &nameBytes, "describe column name copy") != TCL_OK) {
            CONN_GATE_LEAVE(co);
...
int Oradpi_Cmd_Desc(void *cd, Tcl_Interp *ip, Tcl_Size objc, Tcl_Obj *const objv[]) {
    ...
    for (Tcl_Size ci = 0; ci < tl; ci++) {
        ...
        if (ch == '.') {
            dotCount++;
            if (dotCount > 1)
                return Oradpi_SetError(ip, (OradpiBase *)co, -1, "object name contains too many '.' separators (max: schema.object)");
            if (ci + 1 < tl && tab[ci + 1] == '.')
    ...
    for (uint32_t i = 0; i < ncols; i++) {
        ...
        Tcl_IncrRefCount(entry);
        LAPPEND_GOTO(ip, entry, Tcl_NewStringObj(cols[i].name, (Tcl_Size)cols[i].nameLen), code, entry_err);
        LAPPEND_GOTO(ip, entry, Tcl_NewStringObj(cols[i].oracleTypeNum == DPI_ORACLE_TYPE_NUMBER ? "NUMBER" : "OTHER", -1), code, entry_err);
        LAPPEND_GOTO(ip, res, entry, code, entry_err);
        Tcl_DecrRefCount(entry);
        continue;
    entry_err:
